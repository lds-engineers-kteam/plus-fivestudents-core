<?php
function plus_view_monthlyreports_page(){
	global $MOODLESESSION;
	require_once(plugin_dir_path(__DIR__)."/partials/includes/moodlesession.php");
	require_once(plugin_dir_path(__DIR__)."/pages/plus-view-monthlyreport.php");
	$navbar_el = navbar();
	$settings_panel_el = settings_panel();
	$sidebar_el = sidebar();
	$main_panel_el = plus_view_monthlyreport();
	$footer_el = footer();
...
function loggedin_check_plus_view_monthlyreports(){

	if ( is_user_logged_in() && current_user_can('manage_plusgroups')) {
		return plus_view_monthlyreports_page();
	}else{
		// return plus_redirect(site_url().'/');
		// return '<script> location.href="'. site_url().'/"; </script>';
	}
}
 add_shortcode('plus-view-monthlyreport','loggedin_check_plus_view_monthlyreports');
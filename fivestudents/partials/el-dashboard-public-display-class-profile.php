<?php
function plus_view_classProfile(){
	global $MOODLESESSION;
	require_once(plugin_dir_path(__DIR__)."/partials/includes/moodlesession.php");
	require_once(plugin_dir_path(__DIR__)."/pages/plus-view-class-profile.php");
	$navbar_el = navbar();
	$settings_panel_el = settings_panel();
	$sidebar_el = sidebar();
	$main_panel_el = plus_classProfile();
	$footer_el = footer();
...
function loggedin_check_plus_classProfile(){

	if (is_user_logged_in() && current_user_can('manage_plushomework')) {
		return plus_view_classProfile();
	}else{
		return plus_redirect(site_url().'/');
	}
}
 add_shortcode('plus-class-profile','loggedin_check_plus_classProfile');